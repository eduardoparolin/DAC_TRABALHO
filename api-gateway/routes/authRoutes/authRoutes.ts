...
authRoutes.post("/login", authValidator(loginSchema), async (c) => {
  try {
    ...
    const { authServiceUrl, clientServiceUrl } = getServiceUrls();
    ...
    if (!response.ok) {
      console.log("Erro na autenticação:", responseData);
      return c.json(
        { error: responseData.message || "Erro na autenticação" },
        responseData.status || response.status
      );
    }

    if (responseData.usuario.role === "CLIENTE") {
      const token = responseData.accessToken;
      const headers = new Headers();
      if (token) {
        headers.set("Authorization", `Bearer ${token}`);
      }
      console.log(`${clientServiceUrl}/clientes/${responseData.usuario.id}`);
      const clientResponse = await fetch(
        `${clientServiceUrl}/clientes/${responseData.usuario.id}`,
        { headers }
      );
      console.log(clientResponse.status);
      if (clientResponse.ok) {
        const clientData = await clientResponse.json();
        responseData.usuario = { ...responseData.usuario, ...clientData };
      }
    }
    const mappedData = {
      ...responseData,
