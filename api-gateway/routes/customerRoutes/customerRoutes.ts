...
const enrichClientWithAccount = async (
  ...
  try {
    const accountResponse = await fetchWithAuth(
      c,
      `${bankAccountServiceUrl}/query/contas/${accountId}`
    );
    ...
    return {
      ...clientData,
      contaDetalhes: accountData,
    };
  } catch (error) {
    console.error("Erro ao enriquecer cliente com dados da conta:", error);
  }

  return clientData;
};

const enrichAccountWithManager = async (
  c: Context<{ Variables: AppVariables }>,
  clientData: any
) => {
  if (!clientData || !clientData.contaDetalhes) {
    return clientData;
  }
  ...
  const managerId = clientData.contaDetalhes.gerente;
  ...
  try {
    const managerResponse = await fetchWithAuth(
      c,
      `${managerServiceUrl}/manager/id/${managerId}`
    );
    ...
    return {
      ...clientData,
      gerenteDetalhes: managerData,
    };
  } catch (error) {
    console.error("Erro ao enriquecer cliente com dados do gerente:", error);
  }

  return clientData;
};

customerRoutes.get("/perfil/:cpf", authMiddleware, async (c) => {
  try {
    const { cpf } = c.req.param();

    if (!cpf) {
      return c.json({ error: "CPF não fornecido" }, 400);
    }
    ...
    clientData = await enrichClientWithAccount(c, clientData);

    clientData = await enrichAccountWithManager(c, clientData);
    ...
    const response = {
      ...
      conta: clientData.contaDetalhes
        ? {
            numero: clientData.contaDetalhes.numero,
            saldo: clientData.contaDetalhes.saldo,
            limite: clientData.contaDetalhes.limite,
            status: clientData.contaDetalhes.status,
            criacao: clientData.contaDetalhes.criacao,
          }
        : null,
      gerente: clientData.gerenteDetalhes
        ? {
            cpf: clientData.gerenteDetalhes.cpf,
            nome: clientData.gerenteDetalhes.name,
            email: clientData.gerenteDetalhes.email,
            tipo: clientData.gerenteDetalhes.type,
          }
        : null,
    };
...
customerRoutes.get(
  ...
  async (c) => {
    ...
    if (!filtro) {
      const roleValidation = checkRole(c, "ROLE_GERENTE", "ROLE_ADMINISTRADOR");
      ...
      const managersResponse = await fetchWithAuth(
        ...
        `${managerServiceUrl}/manager/gerentes`,
        {
          ...
          body: JSON.stringify({
            managerIds: [...accountsMap.values()].map((a) => a.gerente),
          }),
        }
      );
      let managers = [];
      ...
      const clientsMapped = clients.map((client: any) => {
        const account = client.accountId
          ? accountsMap.get(String(client.accountId))
          : null;
        const myManager = managers.find(
          (m: any) => String(m.id) === String(account.gerente)
        );
        return {
    ...
    switch (filtro) {
      ...
      case "adm_relatorio_clientes": {
        ...
        const clis = clientsWithAccountsAndManagers
          .map((client: any) => {
            return {
              ...client,
              nome: client.name,
  ...
  async (c) => {
    try {
      ...
      return c.json(
        {
          message:
            "Aprovação iniciada com sucesso. Senha será enviada por email.",
